...
| 7 | `src/BotCore/Features/FeaturePublisher.cs` | Hardcoded publish interval and missing latency telemetry | Configuration-driven interval + FAIL-CLOSED validation + latency logging | `var publishInterval = TimeSpan.FromSeconds(30)` hardcoded with fallback | FAIL-CLOSED config validation: `throw InvalidOperationException` if missing/invalid + publish latency telemetry | ✅ |
...
| 10 | `src/adapters/topstep_x_adapter.py` | Fail-open integration and missing retry policies | Implemented fail-closed defaults + centralized retry policies + PRODUCTION RULE COMPLIANCE | Hardcoded retry defaults + partial failures allowed | AdapterRetryPolicy with FAIL-CLOSED config validation + required environment variables | ✅ |
...
| 16 | `src/BotCore/Services/ProductionKillSwitchService.cs` | Safety module alignment: ProductionKillSwitchService doesn't implement IKillSwitchWatcher interface | Added IKillSwitchWatcher implementation to ProductionKillSwitchService for compatibility | No interface implementation | Implements IKillSwitchWatcher with events and async methods | ✅ |
| 19 | `src/TopstepAuthAgent/TopstepAuthAgent.cs` | Secret logging risk: Auth failures logged response body containing sensitive data | Removed response body logging from auth failures to prevent secret exposure | `throw new HttpRequestException($"Auth {(int)resp.StatusCode} {resp.StatusCode}: {body}")` | `throw new HttpRequestException($"Auth {(int)resp.StatusCode} {resp.StatusCode}: Authentication failed")` | ✅ |
| 20 | `src/UnifiedOrchestrator/Services/EnhancedAuthenticationService.cs` | Missing guardrail halt on auth failures per audit requirements | Added EmergencyStopSystem integration to trigger guardrail halts on authentication failures | Authentication failures only logged errors | Authentication failures trigger `TriggerAuthenticationFailureGuardrailHalt()` with emergency stop | ✅ |
