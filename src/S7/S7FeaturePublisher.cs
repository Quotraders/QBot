...
namespace TradingBot.S7
{
    ...
    public sealed class S7FeaturePublisher : IHostedService, IDisposable
    {
        ...
        private const decimal FeatureActiveValue = 1.0m;
        private const decimal FeatureInactiveValue = 0.0m;
        ...
        private void PublishCrossSymbolFeatures(object snapshot, DateTime timestamp, string telemetryPrefix)
        {
            ...
            _featureBus.Publish("CROSS", timestamp, $"{telemetryPrefix}.actionable", ((dynamic)snapshot).IsActionable ? FeatureActiveValue : FeatureInactiveValue);
        ...
        private void PublishFusionTags(object snapshot, DateTime timestamp)
        {
            ...
            if (_config!.EnableFusionTags)
            {
                ...
                foreach (var fusionTag in fusionTags)
                {
                    ...
                    {
                        _featureBus!.Publish("FUSION", timestamp, fusionTag.Key, (decimal)parsedValue);
                    }
                }
            }
        }
        ...
        private void PublishCoreFeatures(string symbol, DateTime timestamp, string telemetryPrefix, object featureTuple)
        {
            ...
            _featureBus.Publish(symbol, timestamp, $"{telemetryPrefix}.signal_active", tuple.IsSignalActive ? FeatureActiveValue : FeatureInactiveValue);
        }
        ...
        private void PublishEnhancedFeatures(string symbol, DateTime timestamp, string telemetryPrefix, object featureTuple)
        {
            ...
            _featureBus.Publish(symbol, timestamp, $"{telemetryPrefix}.dispersion_boosted", tuple.IsDispersionBoosted ? FeatureActiveValue : FeatureInactiveValue);
            _featureBus.Publish(symbol, timestamp, $"{telemetryPrefix}.dispersion_blocked", tuple.IsDispersionBlocked ? FeatureActiveValue : FeatureInactiveValue);
