...
public class NeuralUcbExtended : IDisposable
{
    ...
    private const decimal HighConfidenceThreshold = 0.70m;   // High confidence threshold for selection
    ...
    private static decimal CalculateBracketVolatility(MarketContext marketContext)
    {
        ...
        var volumeVolatility = marketContext.Volume > 0 
            ? (decimal)Math.Min((double)marketContext.Volume / 1000000.0, 1.0) // Normalize volume
            : 0m;
