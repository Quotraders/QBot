...
internal class TradingOrchestratorService : BackgroundService, ITradingOrchestrator
{
    ...
    private async Task ProcessAlwaysLearningTradingAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            var marketContext = await CreateEnhancedMarketContextAsync(cancellationToken).ConfigureAwait(false);
            
            // Get unified trading decision (NEVER HOLD)
            var decision = await GetUnifiedTradingDecisionAsync(marketContext, cancellationToken).ConfigureAwait(false);
            ...
            if (decision != null)
            {
                ...
                var executionResult = await ExecuteUnifiedTradeAsync(decision, cancellationToken).ConfigureAwait(false);
        ...
        try
        {
            ...
            if (_masterOrchestrator != null)
            {
                try
                {
                    ...
                    var botCoreDecision = await CallMasterOrchestratorAsync(marketContext, cancellationToken).ConfigureAwait(false);
            ...
            if (_enhancedBrain != null)
            {
                return await GetEnhancedBrainDecisionAsync(marketContext, cancellationToken).ConfigureAwait(false);
            }
            
            // Priority 3: Unified Trading Brain (Neural UCB + CVaR-PPO)
            return await GetUnifiedBrainDecisionAsync(marketContext, cancellationToken).ConfigureAwait(false);
        }
        catch (Exception ex)
        {
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            var bars = await CreateRealBarsFromContextAsync(marketContext, cancellationToken).ConfigureAwait(false);
            var risk = CreateRiskEngine();
            
            var brainDecision = await _tradingBrain.MakeIntelligentDecisionAsync(
                "ES", env, levels, bars, risk, cancellationToken).ConfigureAwait(false);
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                var initResult = await _topstepXAdapter.InitializeAsync(cancellationToken).ConfigureAwait(false);
                if (!initResult)
                {
            ...
            var currentPrice = await _topstepXAdapter.GetPriceAsync(decision.Symbol, cancellationToken).ConfigureAwait(false);
    ...
    private async Task<TradingBot.Abstractions.MarketContext> CreateEnhancedMarketContextAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            var healthData = await _topstepXAdapter.GetHealthScoreAsync(cancellationToken).ConfigureAwait(false);
    ...
    private async Task<IList<Bar>> CreateRealBarsFromContextAsync(TradingBot.Abstractions.MarketContext context, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (_topstepXAdapter.IsConnected)
            {
                var marketData = await topstepXClient.GetMarketDataAsync(context.Symbol, cancellationToken).ConfigureAwait(false);
    ...
    public async Task ExecuteESNQTradingAsync(WorkflowExecutionContext context, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (_enhancedBrain != null)
            {
                ...
                var realMarketContext = await CreateRealMarketContextForEnhancedBrain("ES", cancellationToken).ConfigureAwait(false);
                ...
                var enhancedDecision = await _enhancedBrain.MakeEnhancedDecisionAsync(
                    "ES", marketContext, availableStrategies, cancellationToken).ConfigureAwait(false);
            ...
            else if (_tradingBrain.IsInitialized)
            {
                ...
                var env = await CreateRealEnvironmentAsync("ES", cancellationToken).ConfigureAwait(false);
                var levels = await CreateRealLevelsAsync("ES", cancellationToken).ConfigureAwait(false);
                var bars = await GetRealBarsAsync("ES", 10, cancellationToken).ConfigureAwait(false);
                var riskEngine = CreateRiskEngineFromRealConfig();
                
                // Get intelligent decision from the brain
                var brainDecision = await _tradingBrain.MakeIntelligentDecisionAsync(
                    "ES", env, levels, bars, riskEngine, cancellationToken).ConfigureAwait(false);
            ...
            else
            {
                ...
                var marketContext = await CreateMarketContextFromWorkflowAsync(context, cancellationToken).ConfigureAwait(false);
                var mlDecision = await _intelligenceOrchestrator.MakeDecisionAsync(marketContext, cancellationToken).ConfigureAwait(false);
    ...
    private async Task<TradingBot.Abstractions.MarketContext> CreateMarketContextFromWorkflowAsync(WorkflowExecutionContext context, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var realContext = await CreateRealMarketContextForEnhancedBrain(symbol, cancellationToken).ConfigureAwait(false);
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            var decision = await _masterOrchestrator!.MakeUnifiedDecisionAsync("ES", localMarketContext, cancellationToken).ConfigureAwait(false);
